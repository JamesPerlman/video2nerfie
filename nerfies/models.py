...
class NerfModel(nn.Module):
    ...
    def setup(self):
        ...
        self.point_encoder = model_utils.vmap_module(
            modules.SinusoidalEncoder, num_batch_dims=2)(num_freqs=self.num_nerf_point_freqs)
        self.viewdir_encoder = model_utils.vmap_module(
            modules.SinusoidalEncoder, num_batch_dims=1)(num_freqs=self.num_nerf_viewdir_freqs)

        if self.use_appearance_metadata:
            self.appearance_encoder = glo.GloEncoder(
                num_embeddings=self.num_appearance_embeddings, features=self.num_appearance_features)

        if self.use_camera_metadata:
            self.camera_encoder = glo.GloEncoder(
                num_embeddings=self.num_camera_embeddings, features=self.num_camera_features)
    ...
    def get_condition_inputs(self, viewdirs, metadata, metadata_encoded=False):
        ...
        if self.use_appearance_metadata:
            if metadata_encoded:
                ...
                appearance_code = self.appearance_encoder(
                    metadata['appearance'])
    ...
    def render_samples(self,
        ...
        if use_warp:
            ...
            warp_metadata = jnp.broadcast_to(warp_metadata[:, jnp.newaxis, :],
                                             shape=(*points.shape[:2], metadata_channels))
            warp_out = self.warp_field(points,
                                       warp_metadata,
                                       warp_extra,
                                       use_warp_jacobian,
                                       metadata_encoded)
    ...
    def __call__(self,
        ...
        z_vals, points = model_utils.sample_along_rays(self.make_rng('coarse'),
                                                       origins,
                                                       directions,
        ...
        coarse_ret = self.render_samples('coarse',
                                         points,
                                         z_vals,
                                         directions,
                                         viewdirs,
        ...
        if self.num_fine_samples > 0:
            ...
            z_vals, points = model_utils.sample_pdf(self.make_rng('fine'),
                                                    z_vals_mid,
                                                    ...
                                                    self.use_stratified_sampling)
            out['fine'] = self.render_samples('fine',
                                              points,
                                              z_vals,
                                              directions,
                                              viewdirs,
...
def construct_nerf(key,
    ...
    model = NerfModel(num_coarse_samples=num_coarse_samples,
